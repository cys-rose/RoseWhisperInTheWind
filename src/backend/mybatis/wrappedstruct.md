...
在 MyBatis 中有什么用呢？在 MyBatis 的执行过程中其作用是在拿到查询结果后，根据其类名拿到反射器对象，并将 DB 中查询到的值填充到这个类里。而**意义**是什么？答案就是：**避免重复使用反射**，每个类只需要一次反射，就拿到所有信息后并保留下来。所以说 `MetaObject` 就是一个反射工具类。
